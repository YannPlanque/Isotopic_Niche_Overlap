...
##     at their Southern European limit range (Eastern English Channel)". Submitted
##
...
lapply(c("coda", "mgcv", "mvtnorm", "dplyr",
         "reshape", "ggplot2", "ggthemes",
...
### I // Data #############################################################################################
  ## 1 / Direction 
  Direction <- ".../Script_Planque et al_[Isotopic_Niche_Overlap]"
  ...
  iso_data %>% group_by(IND, spp) %>%
    summarize(nb_seg = n(), d13d13C_mean = mean(d13C), d15d15N_mean = mean(d15N), rho = cor(d13C, d15N)) )
...
######### Bayesian model taking intra- and inter-individual variability in iso values #####################
  ...
  */ 
  ...
  # d13C
  ID$pred_d13d13C_SD <- apply(rstan::extract(fit, "iso_hat")$iso_hat[, ID$IND, 1], 2, sd)
  # d15N
  ID$pred_d15d15N_SD <- apply(rstan::extract(fit, "iso_hat")$iso_hat[, ID$IND, 2], 2, sd)
  ...
  ## 5 / Extract interindividual variance in species' isotopic niches (posterior omega parameter)
  # For d13C
  ...
  # Probabilities
  # Probability of interindividual variability in d13C to be higher for Pv (harbour seaks) than for Hg (grey seals)
...
rm(Points_prov, Points_provis, Points_unif, Plot_02_probabilities)
save.image(paste(Direction, "Output", "hmodel_iso_Overlap1.RData", sep = "/"), safe = TRUE)
#load(paste(Direction, "Output", "hmodel_iso_Overlap1.RData", sep = "/"))
